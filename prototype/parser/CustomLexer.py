...
from prototype.parser.Errors import IndentationErr
from prototype.parser.PrototypeLexer import PrototypeLexer
...
class CustomLexer(PrototypeLexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == Token.EOF and len(self.indents) != 0:
            ...
            i = len(self.tokens) - 1
            while i >= 0:
                if self.tokens[i].type == PrototypeParser.EOF:
                    del self.tokens[i]
                i -= 1

            # First emit an extra line break that serves as the end of the statement
            self.emitToken(self.commonToken(PrototypeParser.NEWLINE, '\n'))
            ...
            self.emitToken(self.commonToken(PrototypeParser.EOF, "<EOF>"))
    ...
    def createDedent(self):
        dedent = self.commonToken(PrototypeParser.DEDENT, "")
    ...
    def newLineAction(self):
        ...
        if indent == previous:
            ...
            self.emitToken(self.commonToken(PrototypeParser.INDENT, spaces))
        else:
